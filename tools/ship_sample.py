...
- `replay/frames/*.webp` — the {frames} flycam frames, re-encoded from the original PNGs to WebP q80 (roughly 8x smaller). The page prefers `.webp` and falls back to `.png`, so a full-fidelity build also works. Every score and statistic is computed from the exact run logs.
- `route.json`, `summary.json` — byte-exact provenance copies.

Watch it:

    cd {out}/replay && python3 -m http.server 8080
...
def ship(run_dir, out_dir):
    ...
    with open(os.path.join(out_dir, "README.md"), "w") as fh:
        fh.write(SAMPLE_README.format(out=os.path.normpath(out_dir), frames=len(pngs)))
